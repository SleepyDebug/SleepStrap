...
using SleepStrap.Models.APIs.Roblox;
...
namespace SleepStrap
{
    public class Bootstrapper
    {
        ...
        private async Task<bool> ApplyModifications()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, "Failed to prepare blurry texture settings");
                App.Logger.WriteException(LOG_IDENT, ex);
                success = false;
            }
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, "Failed to prepare the RTX shine texture");
                App.Logger.WriteException(LOG_IDENT, ex);
                success = false;
            }

            // Gallery archives often store PNG images with a .tex extension. Ensure
            // the selected preset is materialized as mipmapped BC1 DDS files before
            // copying modifications into the active Roblox version.
            try
            {
                VisualModService.EnsureSelectedSkyboxReady();
            }
            catch (Exception ex)
            {
                App.Logger.WriteLine(LOG_IDENT, "Failed to prepare the selected skybox texture files");
                App.Logger.WriteException(LOG_IDENT, ex);
                success = false;
            }
            ...
            foreach (string fileLocation in previouslyAppliedMods)
            {
                if (currentModFiles.Contains(fileLocation))
                    continue;
                ...
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Failed to remove deployed modification ({fileLocation})");
                    App.Logger.WriteException(LOG_IDENT, ex);
                    modFolderFiles.Add(fileLocation);
                    success = false;
                    managedTextureRepairFailed = true;
                    continue;
                }

                var packageMapEntry = PackageDirectoryMap.SingleOrDefault(x =>
                    !String.IsNullOrEmpty(x.Value) &&
                    fileLocation.StartsWith(x.Value, StringComparison.OrdinalIgnoreCase));
            ...
            if (repairManagedTextures && !managedTextureRepairFailed)
            {
                Directory.CreateDirectory(Path.GetDirectoryName(managedTextureRepairMarker)!);
                File.WriteAllText(managedTextureRepairMarker, App.Version);
            }
